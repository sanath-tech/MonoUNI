...
def decode_detections(dets, info, calibs, denorms,cls_mean_size, threshold):
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            locations = denorms[i].object_center2ground_center(locations.reshape(3,1),dimensions[0]).reshape(-1)
...
#two stage style    
def extract_dets_from_outputs(outputs, calibs,K=50):
    # get src outputs
