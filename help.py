...
def load_inference_data(detection_entries):
    """
    load detection data of kitti format
    """
    data = []
    class_name =  ['car','big_vehicle','pedestrian','cyclist']
    for index, entry in enumerate(detection_entries):
        # Assuming the structure: [cls_id, alpha, x1, y1, x2, y2, h, w, l, X, Y, Z, ry, score]
        # cls_id, alpha = entry[0], entry[1]
        # x1, y1, x2, y2 = entry[2], entry[3], entry[4], entry[5]
        # h, w, l = entry[6], entry[7], entry[8]
        # X, Y, Z = entry[9], entry[10], entry[11]
        # ry, score = entry[12], entry[13]
        
        t_data = Data()
        t_data.obj_type = class_name[int(entry[0])]  # assuming cls_id corresponds to the object type
        t_data.truncation = 0.0  # assuming no truncation information
        t_data.occlusion = 0  # assuming no occlusion information
        t_data.obs_angle = entry[1]  # observation angle [rad]
        t_data.x1 = int(entry[2])  # left   [px]
        t_data.y1 = int(entry[3])  # top    [px]
        t_data.x2 = int(entry[4])  # right  [px]
        t_data.y2 = int(entry[5])  # bottom [px]
        t_data.h = entry[6]  # height [m]
        t_data.w = entry[7]  # width  [m]
        t_data.l = entry[8]  # length [m]
        t_data.X = entry[9]  # X [m]
        t_data.Y = entry[10]  # Y [m]
        t_data.Z = entry[11]  # Z [m]
        t_data.yaw = entry[12]  # yaw angle [rad]
        t_data.score = entry[13]  # detection score
        t_data.detect_id = index
        data.append(t_data)
...
def show_box_with_roll(img,detection_file,extrinsic_file,denorm_file,calfile,inference,network=True, thresh=0.5, projectMethod='World'):
    ...
    color_list = {'car': (0, 0, 255),
            ...
            'barrow': (255, 0, 128)}    
    if network==True:
        result=load_inference_data(inference)
    else:
